...
else if(isset($_REQUEST['viewOpen'])&& $_REQUEST['viewOpen']=='true') //weekday starts from monday=0
{
	$R_ID = mysql_query("SELECT * FROM `site_timing`,`rest_delivery_area` where site_timing.resid=rest_delivery_area.rest_id and dateday=WEEKDAY(CURDATE()) and ((CURTIME() between open1 and close1) or (CURTIME() between open2 and close2) or (CURTIME() between open3 and close3)) and da_id='".$RDa_id ['id']."' order by rest_delivery_area.rpriority DESC");
}
		...
		<div class="cd-tab-filter-wrapper visible-sm visible-md visible-lg">
			<div class="cd-tab-filter">
				<ul class="cd-filters cf">
					...
					<li class="filter" data-filter=""><i class=" fa fa-clock-o"style="color:#fff;"></i><a href="#0" data-type="color-1" onclick="sortSet('Open');">Now Open</a></li>
		...
		<div class="cd-tab-filter-wrapper visible-xs">
			<div class="cd-tab-filter">
				<ul class="cd-filters cf">
					...
					<li class="filter" data-filter="" ><a href="#0" data-type="color-1" onclick="sortSet('Open');">Now Open</a></li>
		...
		<section class="cd-gallery">
			...
			<?php  } ?>
				...
				<!---- Card Service Information ---> 
						...
						<b><a href="<?php echo setRestUrl($rss['id'],$rss['name']); ?>" title="<?php echo $rss['name']; ?>" style="color:#222;"><?php if (strlen($rss['name']) >= 21) { echo substr($rss['name'], 0, 21). "..."; } else { echo $rss['name']; } ?></a></b> 

						<?php if($rss['flash']==1) { echo '<img src="images/new.gif" width="26" height="12" style="margin-top: -10px;">'; }?><br/>
						<?php if($rss['offers']==1) { echo '<img src="images/new.gif" width="26" height="12" style="margin-top: -10px;">'; }?><br/>
...
<?php } ?>
						...
						<?php if($rss['sponsered']==1) { echo '<span class="label label-primary">SPONSORED</span>';   } 
									else if($rss['fzrecommend']==1) { echo '<span class="label label-primary">FZRECOMMENDED</span>';   }//'<span class="label label-tra" style="color:#ff554e">OFFERS</span>'; ?>
						</td></tr>
						<?php
						...
						<tr><td colspan="3">Delivery Fee : <?php if ( $Dfees['dfees'] <= '0' ) { echo "FREE"; } else { echo setPrice($Dfees['dfees']); } ?> <?php if ( $rss['delivery_type'] == '2' ){echo '<span class="label label-danger"><i class="fa fa-bolt" style="color:#fff;" title="FzDelivery"></i></span>'; }?> </td></tr>
				...
				<div class="cd-filter-block">
					...
					<div class="cd-filter-content">
						...
						<select class="filter" name="selectThis" id="selectThis" onchange="sortSet(this.value);">
								...
								<option value="ByMinOrderValue">Minimum Order Value</option>
								<option value="ByDeliveryFee">Delivery Fee</option>
							</select>
						</div> <!-- cd-select -->
				...
				<div class="cd-filter-block">
					...
					<li>
						...
						<li>
							...
							<label class="radio-label" for="radio5">FzRecommended</label>
						</li>
...
function  sortSet(value)
{
	...
	{
		var location = window.location.pathname+"?service="+<?php if(isset($_SESSION['myservice'])){ echo $_SESSION['myservice']; } ?>+"&viewOpen=true";
		window.location.href=location;
	}
}
</script>
