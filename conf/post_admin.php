...
if ( $cmd == "sys_login" )
{
    ...
    {   $query=mysql_query( "SELECT username, City, Delivery_Type  FROM adminUsers WHERE username='".safe( $_REQUEST['username'] )."' AND  password='".md5(safe( $_REQUEST['password'] ))."'" );
        ...
        if ( $num_rows!=1 )
        {
            ...
            echo "<script> go('./welcome.php')</script>";
        }
    }
}
...
if ( $cmd == "save_rest" )
{
    ...
    else
    {
        ...
        $data['packing_fee_type'] = $_REQUEST['packing_fee_type'];
        $data['packing_fee'] = $_REQUEST['packing_fee'];
        $data['custom_qty'] = $_REQUEST['custom_qty'];
