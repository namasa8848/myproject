...
<?php
   ...
   $Addsql="";
    ...
    $query=mysql_query("SELECT * FROM delivery_expense WHERE agent_id=".$_REQUEST['rid'].$addsql);
    $Q1=mysql_query("SELECT * FROM agent_login WHERE id=".$_REQUEST['rid']);
    $Q=mysql_fetch_array($Q1);

    echo " " . $Q['Name'] . " <br/><small>Phone: ".$Q['gsm']." City: ".$Q['City']." Salary: ".$Q['Salary']; 
?>
...
<form method="get" action="<?=$_SERVER['PHP_SELF']?>">
    Agent ID : <input type="text" name="rid" id="rid" value="<?=$_REQUEST['rid']?>" style="width:50px"> &nbsp; &nbsp; 
...
<table width="100%">
  ...
  <tbody>  
    <?php $count=0;  while($row=mysql_fetch_array($query)){ 
      ...
      $COD="SELECT sum(order_total) as A, sum(service_fee) as B from orders WHERE date(orderdate)='".$row['Date']."' and status_agent='Delivered:".$_REQUEST['rid']."' and paymenttype='COD'";
      $OP="SELECT sum(order_total) as A, sum(service_fee) as B from orders WHERE date(orderdate)='".$row['Date']."' and status_agent='Delivered:".$_REQUEST['rid']."' and paymenttype='ONLINE PAYMENT'";
      $SQL="SELECT count(id) as A, sum(order_total) as B from orders WHERE date(orderdate)='".$row['Date']."' and status_agent='Delivered:".$_REQUEST['rid']."'";
      $a=mysql_query($COD);
      $b=mysql_query($OP);
      $c=mysql_query($SQL);
      ...
      $ANT=$row['Amount_Rcvd_NoTip']-$row['Petrol']-$row['Food']-$row['Extra'];
    ?>
    <tr  class="<?php echo $class; ?>">
      ...
      <td ><center><?php echo $row['Petrol'];?></center></td>      
      <td ><center><?php echo $row['Food'];?></center></td>      
      <td ><center><?php echo $row['Extra'];?></center></td> 
      <td ><center><?php echo $row['Extra_Comments'];?></center></td> 
      <td ><center><?php echo $row['Tips'];?></center></td> 
      <td ><center><?php echo $row['Advance'];?></center></td> 
      <td ><center><?php 
        if($row['Holiday']==1) echo "Yes";
        else echo "No"; ?> </center></td> 
      <td ><center><?php echo $ANT ?></center></td>     
      <td ><center><?php echo $ANT+$row['Tips']; ?></center></td>    
   </tr>

    <?php  } ?>
    <?php 
      $query=mysql_query("SELECT sum(Petrol) as A, sum(Food) as B, sum(Extra) as C, sum(Tips) as D, sum(Advance) as E, sum(Amount_Rcvd_NoTip) as F, sum(Holiday) as G FROM delivery_expense WHERE agent_id=".$_REQUEST['rid'].$addsql);
      $row=mysql_fetch_array($query);
      $ANT=$row['F']-$row['A']-$row['B']-$row['C'];

      $COD="SELECT sum(order_total) as A, sum(service_fee) as B from orders WHERE status_agent='Delivered:".$_REQUEST['rid']."' and paymenttype='COD'".$Addsql;
      $OP="SELECT sum(order_total) as A, sum(service_fee) as B from orders WHERE status_agent='Delivered:".$_REQUEST['rid']."' and paymenttype='ONLINE PAYMENT'".$Addsql;
      $SQL="SELECT count(id) as A, sum(order_total) as B from orders WHERE status_agent='Delivered:".$_REQUEST['rid']."'".$Addsql;
      $a=mysql_query($COD);
      $b=mysql_query($OP);
      $c=mysql_query($SQL);
